...
uint8_t s1_addr[] = {0x28, 0x3B, 0xD6, 0x95, 0xF0, 0x1, 0x3C, 0xFD}; // Адресс термодатчика DS18B20

DHT dht(DHTPIN, DHTTYPE);            // Создаём DHT термодатчик
LiquidCrystal_I2C lcd(0x27, 16, 2);  // set the LCD address to 0x27 for a 16 chars and 2 line display
MicroDS18B20<DS_PIN> DS18B20_sensor; // Создаем термометр с адресацией
...
GyverNTC therm(NTC_PIN, 10100, 3950, 25, 10000);                  // пин, сопротивление при 25 градусах (R термистора = R резистора!), бета-коэффициент
...
uint8_t memSize = 128;              // размер выделяемой памяти в EEPROM
bool DS, NTC, DHT, BMP;
...
void EEPROMWrite(uint8_t addr) //
{
    EEPROM.write(addr, average);
    EEPROM.commit();                                                                               // записали в EEPROM
    Serial.println(String("EEPROM.write(addr): ") + addr + (", температура average: ") + average); //!//!
}
...
void EEPROMRead()
{
    ...
    uint8_t aa[] = {0, 4, 8, 12, 16, 20, 24, 28, 32, 36, 40, 44, 48, 52, 56, 60, 64, 68, 72, 76, 80, 84, 88, 92};
...
void setup()
{
    ...
    // rtc.setTime(17, 32, 00);        // запись в модуль РЕАЛЬНОГО ВРЕМЕНИ значения ЧАСА, МИНУТ, СЕКУНД
